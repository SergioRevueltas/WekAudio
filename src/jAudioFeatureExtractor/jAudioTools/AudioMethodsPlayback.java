...
import com.srevueltas.core.NotifyingThread;
...
public class AudioMethodsPlayback {
	...
	/**
	 * Plays the samples in the given <code>AudioInputStream</code> to the specified <code>SourceDataLine</code>.
	 *
	 * <p>
	 * The thread performing the playback is returned to the user, so the user has the option of interrupting it if he
	 * or she wishes.
	 *
	 * <p>
	 * Note that the same <code>AudioFormat</code> must be used by both the <code>AudioInputStream</code> and the
	 ...
	 * @throws Exception Throws an exception if the <code>AudioFormat</code> of the audio_input_stream parameter does
	 *             not match that of the source_data_line parameter.
	 */
	public static PlayThread playAudioInputStreamInterruptible(AudioInputStream audio_input_stream,	SourceDataLine source_data_line)
			throws Exception
	{
	 ...
	 *
	 * <p>
	 * This playback is not interruptible by the user, and will continue until the <code>AudioInputStream</code> is
	 * empty. The <code>playAudioInputStreamInterruptible</code> method should be called instead if the user wishes to
	 * be able to interrupt playback.
	 *
	 * <p>
	 * Note that the same <code>AudioFormat</code> must be used by both the <code>AudioInputStream</code> and the
	 * <code>SourceDataLine</code>.
	...
	/**
	 * A thread for playing an <code>AudioInputStream</code> directly to a <code>SourceDataLine</code>. Can be
	 * interrupded using the <code>stopPlaying</code> method.
	 *
	 * <p>
	 * <b>WARNING:</b>Note that the <code>SourceDataLine</code> is closed after playback has terminated. It will need to
	 * be reopened if it is to be used elsewhere.
	 */
	public static class PlayThread extends NotifyingThread
	{
		...
		/**
		 * Constructor that performs standard operations of the Thread class as well as setting the source
		 * <code>AudioInputStream</code>, setting the target <code>SourceDataLine</code> and initializing an
		 * intermediate buffer to hold 1/4 seconds of audio.
		 *
		 * @throws Exception Throws an exception if either parameter is null.
		 * @throws Exception Throws an exception if the <code>AudioFormat</code> of the audio_input_stream parameter
		 *             does not match that of the source_data_line parameter.
		 */
		 ...
		 *
		 * <p>
		 * <b>WARNING:</b>Note that the <code>SourceDataLine</code> is closed after playback has terminated. It will
		 * need to be reopened if it is to be used elsewhere.
		 */
		/*public void run()
		{
