...
public class MainWindow extends JFrame implements ActionListener {
	...
	private void loadGUI() {
		...
		rawWavePanelContainer.setLayout(new MigLayout("", "[:5.00sp:5.00sp,grow][::45.00sp,grow]",
				"[50.87%:109.00:44.94%,grow][][:33.00:40.00][50.29%:168.00px:44.90%,grow,center]"));
	...
	private void abrirWav() {
		...
		if (seleccion == JFileChooser.APPROVE_OPTION) {
			...
			rawWavePanelChannel0 =
					new WavePanel(audioManager.getAudioData()[0], audioManager.getSampleRate(), audioManager
							.getSampleSizeInBits(), channel0Container.getHeight(), channel0Container.getWidth());
			channel0Container.add(rawWavePanelChannel0, BorderLayout.CENTER);
			if (audioManager.getNumChannels() == 2) {
				rawWavePanelChannel1 =
						new WavePanel(audioManager.getAudioData()[1], audioManager.getSampleRate(), audioManager
								.getSampleSizeInBits(), channel1Container.getHeight(), channel1Container.getWidth());
